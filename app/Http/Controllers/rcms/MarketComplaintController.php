...
public function child_capa_devtion(Request $request, $id)
    {
        $cft = [];
        $parent_id = $id;
        $parent_type = "CAPA";
...
// dd( $parent_record );
        ...
        if ($request->child_type == "capa") {
            ...
            return view('frontend.forms.capa', compact('cft','expectedParenRecord', 'old_record', 'parent_short_description', 'parent_initiator_id', 'parent_intiation_date', 'parent_name', 'parent_division_id', 'parent_record', 'record_number', 'due_date', 'parent_id', 'parent_type', 'data_record', 'data'));
        }
        if ($request->child_type == "deviation") {
            ...
            $data_record = Helpers::getDivisionName($data->division_id) . '/' . 'CAPA' . '/' . date('Y') . '/' . str_pad($data->record, 4, '0', STR_PAD_LEFT);
            $expectedParenRecord = Helpers::getDivisionName(session()->get('division')) . "/CAPA/" . date('Y') . "/" . $data->record . "";
            return view('frontend.forms.deviation_new', compact('cft','expectedParenRecord', 'old_record', 'parent_short_description', 'parent_initiator_id', 'parent_intiation_date', 'parent_name', 'parent_division_id', 'parent_record', 'record_number', 'due_date', 'parent_id', 'parent_type', 'data_record', 'data'));
        }
